      ...
      <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg border border-transparent">
        ...
        {errorMsg && (
          <div className="mb-4 p-3 rounded bg-red-100 border border-red-400 text-red-700 flex justify-between items-center">
            <span>{errorMsg}</span>
            <button aria-label="Close error message" onClick={() => setErrorMsg(null)} className="text-xl font-bold leading-none">
              ×
            </button>
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-6" noValidate>
          ...
          <button
            type="submit"
            disabled={loading}
            className={`w-full flex items-center justify-center bg-green-600 hover:bg-green-700 text-white p-3 rounded transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-400 ${
